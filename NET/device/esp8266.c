...
#define ESP8266_WIFI_INFO		"AT+CWJAP=\"one\",\"1234567890\"\r\n"
...
{	
	...
	GPIO_Initure.GPIO_Pin = GPIO_Pin_0;					//WIFI复位
	...
	while(ESP8266_SendCmd("AT\r\n\r", "OK", 200))//测试AT启动
		delay_ms(500);
	sprintf((char *)buff,"AT Init success!");
	OLED_ShowString(0,0,buff,12);
	printf("CWMODE\r\n");

	while(ESP8266_SendCmd("AT+CWMODE=1\r\n", "OK", 200))//设置WiFi模式
		delay_ms(500);
	sprintf((char *)buff,"AT+CWMODE=1");
	OLED_ShowString(0,16,buff,12);
	printf("AT+CWDHCP\r\n");

	while(ESP8266_SendCmd("AT+CWDHCP=1,1\r\n", "OK", 200))//设置DHCP开启
		delay_ms(500);
	sprintf((char *)buff,"AT+CWDHCP=1");
	OLED_ShowString(0,32,buff,12);
	
	printf("CWJAP\r\n");

	while(ESP8266_SendCmd(ESP8266_WIFI_INFO, "GOT IP", 200))//连接路由 
		delay_ms(500);
	sprintf((char *)buff,"GOT IP success!");
	OLED_ShowString(0,48,buff,12);
#endif
	printf("CIPSTART\r\n");

	while(ESP8266_SendCmd(ESP8266_ONENET_INFO, "CONNECT", 200))//esp8266作为client连接到TCP server
		delay_ms(500);
	sprintf((char *)buff,"CONNECT ONENET");
	OLED_ShowString(0,64,buff,12);
